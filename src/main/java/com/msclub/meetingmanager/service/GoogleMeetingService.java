...
*   And to authorize an account, go to the "/api/googlemeet/requestAuthorization" endpoint in a browser
...
public class GoogleMeetingService {
    ...
    public static void init() throws Exception {
        ...
        if(credentialsJSON.trim().length() == 0){
            System.out.println("WARNING - GoogleCredentialsJSON is not set. GoogleMeet feature will not work");
            return;
        }
        ...
        if(refreshToken.trim().length() > 0){
            ...
            System.out.println("WARNING - GoogleRefreshToken is not set, go to /api/googlemeet/requestAuthorization to get a refresh token. (Before this make sure you have 'GoogleRedirectURIHost' environment variable set");
        }
    }
